...
<article class="container mt-28">
    ...
    @if($greeetings->isEmpty())
        <p class="text-center text-xneutral-200 text text-montserrat text-lg">
          no data available
        </p>

    @else
    @foreach ( $greeetings as $greeeting )
        
    

    <div class="grid grid-cols-1 sm:grid-cols-12 gap-8 mt-8">
        <img src="{{ asset('storage/' . $greeeting->image)}}" alt="Rektor B-University" class="w-full h-fit object-cover rounded-[30px] sm:col-span-4" />
